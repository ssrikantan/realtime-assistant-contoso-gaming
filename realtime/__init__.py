...
class RealtimeAPI(RealtimeEventHandler):
    def __init__(self):
        ...
        self.url = f'wss://{DefaultConfig.az_open_ai_endpoint_name}.openai.azure.com/'
        self.api_key = DefaultConfig.az_openai_key
        self.api_version = DefaultConfig.az_openai_api_version
        self.azure_deployment = DefaultConfig.deployment_name
        self.ws = None
...
class RealtimeConversation:
    ...
    EventProcessors = {
        ...
        'response.audio_transcript.done': lambda self, event: self._process_audio_transcript_done(event),
    }
    ...
    """
    srikantan added this function manually
    """
    def _process_audio_transcript_done(self, event):
        item_id = event['item_id']
        content_index = event['content_index']
        transcript = event['transcript']
        item = self.item_lookup.get(item_id)
        if not item:
            raise Exception(f'response.audio_transcript.done: Item "{item_id}" not found')
        item['content'][content_index]['transcript'] = transcript
...
class RealtimeClient(RealtimeEventHandler):
    ...
    def _add_api_event_handlers(self):
        ...
        self.realtime.on("server.response.audio_transcript.done", self._response_audio_transcript_completed_event)
    ...
    def _response_audio_transcript_completed_event(self, event):
        self._process_event(event)
        # if item:
        #     self.dispatch("response.audio_transcript.done", {"item": item})
        # return item
        # self._process_event(event)
        self.dispatch("response.audio_transcript.done", event)
